...
func (i *InventoryHandler) AddProduct(c *gin.Context) {
	var products models.AddInventories

	cat := c.PostForm("category_id")
	products.CategoryID, _ = strconv.Atoi(cat)
	products.ProductName = c.PostForm("product_name")
	products.Size = c.PostForm("size")
	products.Stock, _ = strconv.Atoi(c.PostForm("stock"))
	products.Price, _ = strconv.ParseFloat(c.PostForm("price"), 64)

	file, err := c.FormFile("image")
	if err != nil {
		errorRes := response.ClientResponse(http.StatusBadRequest, "Image was not fetched", nil, err.Error())
		c.JSON(http.StatusBadRequest, errorRes)
		return
	}
	ProductResponse, err := i.InventoryUseCase.AddProduct(products, file)
	if err != nil {
		errRes := response.ClientResponse(http.StatusBadRequest, "product adding failed", nil, err.Error())
		c.JSON(http.StatusBadRequest, errRes)
		return
	}

	successRes := response.ClientResponse(http.StatusOK, "Successfully Added", ProductResponse, nil)
...
func (i *InventoryHandler) ListProducts(c *gin.Context) {
	...
	products_list, err := i.InventoryUseCase.ListProducts()
