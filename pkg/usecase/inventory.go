...
func (i *inventoryUseCase) AddProduct(product models.AddInventories, file *multipart.FileHeader) (models.ProductsResponse, error) {
	if product.ProductName == "" {
		return models.ProductsResponse{}, errors.New("product name should not be empty ")

	}
	if product.Size == "" {
		return models.ProductsResponse{}, errors.New("size should not be empty ")
	}
	if product.CategoryID <= 0 || product.Price <= 0 || product.Stock <= 0 {
		err := errors.New("invalid entry")
		return models.ProductsResponse{}, err
	}
	ok, err := i.catrepo.CategoryExists(product.CategoryID)
	if err != nil {
		return models.ProductsResponse{}, err
	}
	if !ok {
		return models.ProductsResponse{}, errors.New("invalid category id")
	}
	ok, _ = i.repository.CheckInventoryByCatAndName(product.CategoryID, product.ProductName)
...
func (i *inventoryUseCase) ListProducts() ([]models.ProductsResponseDisp, error) {
	productList, err := i.repository.ListProducts()
	if err != nil {
