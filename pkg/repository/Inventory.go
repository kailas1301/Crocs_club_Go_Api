...
func (i *inventoryRepository) AddProduct(product models.AddInventories, url string) (models.ProductsResponse, error) {

	var count int64
	i.DB.Model(&models.Inventories{}).Where("product_name = ? AND category_id = ?", product.ProductName, product.CategoryID).Count(&count)
	if count > 0 {

		return models.ProductsResponse{}, errors.New("product already exists")
	}

	if product.Stock <= 0 || product.Price <= 0 {
		return models.ProductsResponse{}, errors.New("stock and price are not valid")
	}
	...
	var productResponse models.ProductsResponse

	query = "SELECT id,category_id,product_name,size,stock,price FROM inventories where  product_name = ? AND category_id = ?"
	errr := i.DB.Raw(query, product.ProductName, product.CategoryID).Scan(&productResponse).Error

	if errr != nil {
		return productResponse, errors.New("data base error")
	}
	//Adding url to image table

	queryimage := "INSERT INTO images (inventory_id, url) VALUES (?, ?)"
...
func (prod *inventoryRepository) ListProducts() ([]models.ProductsResponseDisp, error) {
...
left JOIN
  ...
  `
	err := prod.DB.Raw(query).Scan(&product_list).Error
	...
	fmt.Println("update details", updatedproductDetails)
...
func (i *inventoryRepository) DeleteInventory(inventoryID string) error {
	...
	_ = i.DB.Exec("DELETE FROM images WHERE inventory_id = ?", id)
...
func (i *inventoryRepository) GetInventory(prefix string) ([]models.ProductsResponse, error) {
	...
	query := `
        SELECT i.id, i.category_id, i.product_name, i.size, i.stock, i.price, ARRAY_AGG(im.url) as image
        FROM inventories i
        LEFT JOIN categories c ON i.category_id = c.id
        LEFT JOIN images im ON i.id = im.inventory_id
        WHERE i.product_name ILIKE '%' || $1 || '%'
        OR c.category ILIKE '%' || $1 || '%'
        GROUP BY i.id, i.category_id, i.product_name, i.size, i.stock, i.price;
    `

	rows, err := i.DB.Raw(query, prefix).Rows()
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&product.ID, &product.CategoryID, &product.ProductName, &product.Size, &product.Stock, &product.Price, &imageURLs); err != nil {
			return nil, err
		}
