...
func (o *orderRepository) GetOrderDetailsBrief() ([]models.CombinedOrderDetails, error) {

	var orderDetails []models.CombinedOrderDetails
	que := `SELECT orders.id AS order_id, orders.final_price, orders.order_status, orders.payment_status, 
	...
	INNER JOIN addresses ON orders.address_id = addresses.id order by orders.id desc `
	err := o.DB.Raw(que).Scan(&orderDetails).Error
